...
models_to_test = [
    "deepseek-v4-flash",  # OpenAI format, fast
    "deepseek-v4-pro",    # OpenAI format, capable
    "glm-5",              # OpenAI format, balanced
    "kimi-k2.5",          # OpenAI format, balanced
    "minimax-m3",         # Anthropic format, capable
    "qwen3.7-max",        # Anthropic format, capable
]

results = {}
for model_name in models_to_test:
    model_info = MODEL_REGISTRY[model_name]
    print(f"Testing {model_name} ({model_info['format']})...", end=" ", flush=True)
    try:
        result = asyncio.run(
            asyncio.wait_for(
                get_layout_decision(
